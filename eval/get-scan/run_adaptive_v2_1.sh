...
python3 "$BENCH_PATH/bench_leveldb.py" \
	--cpu 8 \
	--policy-loader "$POLICY_PATH/cache_ext_adaptive_v2_1.out" \
	--results-file "$RESULTS_PATH/adaptive_v2_1_results.json" \
	--leveldb-db "$DB_PATH" \
...
python3 "$BENCH_PATH/bench_leveldb.py" \
	--cpu 8 \
	--policy-loader "$POLICY_PATH/cache_ext_adaptive_v2_1.out" \
	--results-file "$RESULTS_PATH/adaptive_v2_1_results_mglru.json" \
	--leveldb-db "$DB_PATH" \
	--fadvise-hints "" \
	--iterations "$ITERATIONS" \
	--bench-binary-dir "$YCSB_PATH/build" \
...
echo "ADAPTIVE-V2.1 benchmark completed. Results saved to $RESULTS_PATH."
