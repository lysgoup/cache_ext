...
#include "cache_ext_adaptive_v2.skel.h"
...
char *USAGE =
	"Usage: ./cache_ext_adaptive_v2 --watch_dir <dir> --cgroup_path <path>\n"
	"\n"
	"Enhanced adaptive cache eviction policy with multiple metrics:\n"
...
{
	int ret = 1;
	struct cache_ext_adaptive_v2_bpf *skel = NULL;
	...
	skel = cache_ext_adaptive_v2_bpf__open();
	if (skel == NULL) {
		perror("Failed to open BPF skeleton");
		goto cleanup;
	}

	ret = cache_ext_adaptive_v2_bpf__load(skel);
	if (ret) {
	...
	link = bpf_map__attach_cache_ext_ops(skel->maps.adaptive_v2_ops,
					     cgroup_fd);
	if (link == NULL) {
	...
	printf("========================================\n");
	printf("Enhanced Adaptive Policy v2 Started\n");
	printf("========================================\n");
...
cleanup:
	...
	cache_ext_adaptive_v2_bpf__destroy(skel);
	if (cgroup_fd >= 0)
		close(cgroup_fd);
