...
// ===== 설정 가능한 파라미터 (v2_1: 완화된 임계값) =====
#define HIT_RATE_THRESHOLD 30
#define MIN_SAMPLES 200              // v2: 1000 → v2_1: 200
#define MIN_TIME_IN_POLICY 2000      // v2: 10000 → v2_1: 2000
#define CHECK_INTERVAL 200           // v2: 1000 → v2_1: 200
#define METRIC_PRINT_INTERVAL 100    // 🆕 주기적 메트릭 출력
...
// 🔧 v2_1 수정: 단일 통합 리스트만 사용
static u64 main_list = 0;

// ===== 🆕 Per-inode Sequential Tracking =====
struct seq_tracker {
	u64 last_offset;
	u64 seq_count;
	u64 random_count;
...
static void check_and_switch_policy(void)
{
	...
	// 10. 🔧 v2_1 수정: 모든 윈도우 메트릭 리셋
	reset_window_metrics();
}

// ===== 개별 정책 로직 =====
// 🔧 v2_1 수정: 모두 main_list 사용
...
s32 BPF_STRUCT_OPS_SLEEPABLE(adaptive_v2_1_init, struct mem_cgroup *memcg)
{
	bpf_printk("Adaptive v2.1 policy initializing...\n");

	// 🔧 v2_1 수정: 단일 리스트만 생성
	main_list = bpf_cache_ext_ds_registry_new_list(memcg);
	if (!main_list) {
		bpf_printk("Failed to create main_list\n");
		return -1;
	}
	...
	bpf_printk("Adaptive v2.1 initialized (single-list architecture)\n");
	return 0;
}

void BPF_STRUCT_OPS(adaptive_v2_1_folio_added, struct folio *folio)
{
	...
	// 🆕 v2_1: Per-inode sequential 패턴 감지
...
void BPF_STRUCT_OPS(adaptive_v2_1_folio_accessed, struct folio *folio)
{
	...
	// 🔧 v2_1 수정: 정책에 따라 리스트 이동 (모두 main_list 사용)
	switch (current_policy) {
	case POLICY_MRU:
...
void BPF_STRUCT_OPS(adaptive_v2_1_folio_evicted, struct folio *folio)
{
	u64 key = (u64)folio;
	struct folio_metadata *meta = get_folio_metadata(folio);
...
void BPF_STRUCT_OPS(adaptive_v2_1_evict_folios,
		    ...
		    struct mem_cgroup *memcg)
{
	...
	// 🔧 v2_1 수정: 현재 정책에 맞는 iterate 함수로 main_list 순회
	switch (current_policy) {
	case POLICY_MRU:
		ret = bpf_cache_ext_list_iterate(memcg, main_list,
...
SEC(".struct_ops.link")
struct cache_ext_ops adaptive_v2_1_ops = {
	.init = (void *)adaptive_v2_1_init,
	.evict_folios = (void *)adaptive_v2_1_evict_folios,
	.folio_accessed = (void *)adaptive_v2_1_folio_accessed,
	.folio_evicted = (void *)adaptive_v2_1_folio_evicted,
	.folio_added = (void *)adaptive_v2_1_folio_added,
};
