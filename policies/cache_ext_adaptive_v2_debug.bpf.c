...
// ===== 완화된 파라미터 (DEBUG VERSION) =====
#define HIT_RATE_THRESHOLD 30
#define MIN_SAMPLES 50              // 1000 → 50 (큰 완화)
#define MIN_TIME_IN_POLICY 50       // 10000 → 50 (큰 완화)
#define CHECK_INTERVAL 5            // 1000 → 5 (자주 체크)
...
static u32 decide_best_policy(void)
{
	...
	// 1. Sequential scan 감지 (완화됨: 80% → 30%)
	if (sequential_ratio > 30) {
		bpf_printk("DEBUG: Decision: FIFO (sequential_ratio=%llu%%)\n",
		           sequential_ratio);
		return POLICY_FIFO;
	}

	// 2. One-time scan 감지 (완화됨: 60% → 20%)
	if (one_time_ratio > 20 && avg_hits < 2) {
		bpf_printk("DEBUG: Decision: FIFO (one_time_ratio=%llu%%, avg_hits=%llu)\n",
		           one_time_ratio, avg_hits);
		return POLICY_FIFO;
	}

	// 3. Hot working set (완화됨: 5 → 1)
	if (avg_hits > 1 && one_time_ratio < 30) {
		bpf_printk("DEBUG: Decision: MRU (avg_hits=%llu, one_time_ratio=%llu%%)\n",
	...
	if (avg_reuse_dist > 0 && avg_reuse_dist < 50000) {
		bpf_printk("DEBUG: Decision: LRU (avg_reuse_distance=%llu)\n",
		           avg_reuse_dist);
		return POLICY_LRU;
	}
	...
	if (mru_perf >= fifo_perf && mru_perf >= lru_perf) {
		bpf_printk("DEBUG: Decision: MRU (best past performance: %llu%%)\n", mru_perf);
		return POLICY_MRU;
	} else if (fifo_perf >= lru_perf) {
		bpf_printk("DEBUG: Decision: FIFO (best past performance: %llu%%)\n", fifo_perf);
		return POLICY_FIFO;
	} else {
		bpf_printk("DEBUG: Decision: LRU (best past performance: %llu%%)\n", lru_perf);
		return POLICY_LRU;
	}
}
...
static void check_and_switch_policy(void)
{
	...
	bpf_printk("DEBUG: Policy switch: %d -> %d (hit_rate: %llu%%)\n",
...
s32 BPF_STRUCT_OPS_SLEEPABLE(adaptive_v2_debug_init, struct mem_cgroup *memcg)
{
	bpf_printk("DEBUG: Adaptive v2 (DEBUG VERSION) initializing...\n");
	...
	bpf_printk("DEBUG: Adaptive v2 (DEBUG) initialized with RELAXED thresholds\n");
	return 0;
}

void BPF_STRUCT_OPS(adaptive_v2_debug_folio_added, struct folio *folio)
{
...
void BPF_STRUCT_OPS(adaptive_v2_debug_folio_evicted, struct folio *folio)
{
	u64 key = (u64)folio;
	struct folio_metadata *meta = get_folio_metadata(folio);
...
void BPF_STRUCT_OPS(adaptive_v2_debug_evict_folios,
...
SEC(".struct_ops.link")
struct cache_ext_ops adaptive_v2_debug_ops = {
	.init = (void *)adaptive_v2_debug_init,
	.evict_folios = (void *)adaptive_v2_debug_evict_folios,
	.folio_accessed = (void *)adaptive_v2_debug_folio_accessed,
	.folio_evicted = (void *)adaptive_v2_debug_folio_evicted,
	.folio_added = (void *)adaptive_v2_debug_folio_added,
};
